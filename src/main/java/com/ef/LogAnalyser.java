...
import com.ef.entities.HibernateUtil;
import com.ef.entities.IpAddress;
import com.ef.entities.IpAddressStatusComment;
import com.ef.entities.StatusComment;
...
public class LogAnalyser {
    ...
    public void readFile(String filename) {
        ...
        try (Stream<String> lines = Files.lines(Paths.get(filename))) {
            lines.forEachOrdered(line -> {
                records.add(LogParse.parseEntry(line));
            });
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void mapIpAddressAgainstComment(String startDate, String duration, int threshhold) {
        ...
        List<IpAddressStatusComment> ipAddressStatusComments = new ArrayList<>();
        HashMap<String, List<LogEntry>> records = groupRecordsByIpAddress(startDate, duration, threshhold);
        records.forEach((s, logEntries) -> {
            for (LogEntry logEntry : logEntries) {
                ...
                try {
                    ...
                    StatusComment comment = new StatusComment(logEntry.getStatusCode(), returnComment(logEntry.getStatusCode()));
                    comments.add(comment);
                    session.saveOrUpdate(comment);

                    IpAddress ipAddress = new IpAddress(logEntry.getIpAddress(), logEntry.getAccessTime());
                    session.saveOrUpdate(ipAddress);

                    IpAddressStatusComment ipAddressStatusComment = new IpAddressStatusComment(ipAddress, comment);
                    ipAddressStatusComments.add(ipAddressStatusComment);
                    session.saveOrUpdate(ipAddressStatusComment);
        ...
        groupRecordsByIpAddress(startDate, duration, threshhold)
            ...
            e.getValue().stream().distinct()
                ...
                LogEntry logEntry = new LogEntry(address.getAddress(), returnComment(leSet.getKey()));
        ...
        System.out.println("  IP ADDRESS     ||      STATUS/COMMENT  ");
        System.out.println("________________________________________");
        logEntries.stream().forEachOrdered(c -> System.out.println(
                c.getIpAddress() + " : " + c.getComment()
        ));
        System.out.println("________________________________________");
